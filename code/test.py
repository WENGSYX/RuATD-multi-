...
CFG = { #训练的参数配置
    'seed': 1,
    'model': r'./paperlog/mdeberta_instrustloss/5_0.6220538329226907_model', #预训练模型
    'max_len': 512, #文本截断的最大长度
    'epochs': 16,
    'train_bs': 11, #batch_size，可根据自己的显存调整
    'valid_bs': 100,
    'lr': 1e-5, #学习率
    'num_workers': 0,
...
def collate_fn(data):
    ...
    for x in data:
        ...
        text = tokenizer(text, padding='max_length', truncation=True, max_length=CFG['max_len'])
...
model = DebertaV2ForSequenceClassification.from_pretrained(CFG['model'],num_labels = 14)  # 模型
...
val_loss, val_acc, pred = test_model(model, val_loader,0)


test = pd.read_csv('test.csv')
status_dict = pd.read_pickle('status.pk')
result = []
for i in range(len(test)):
    result.append({'Id':test.iloc[i]['Id'],'Class':status_dict[pred[i].argmax(0).item()]})
